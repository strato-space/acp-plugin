...
export const ToolItem = memo(function ToolItem({
  ...
  const counters = useMemo(() => {
    ...
    const pill = (label: string, className?: string) => (
      <span
        className={cn(
          "px-1.5 py-0.5 rounded-full bg-muted/40 text-foreground border border-border/50 text-[10px]",
          className
        )}
      >
        {label}
    ...
    return (
      <span className="flex items-center gap-1.5">
        ...
        {failed > 0 ? pill(`${failed} failed`, "bg-destructive/10 text-destructive border-transparent") : null}
      </span>
    );
      ...
      <ToolContent>
        ...
        {truncatedOutput && (
          <div className="px-3 pb-3">
            ...
            >
              {showAnsi ? (
                <div className="overflow-x-auto rounded-md bg-muted/50 p-2 text-xs font-mono max-h-[200px] overflow-y-auto">
