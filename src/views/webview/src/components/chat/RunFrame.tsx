...
export const RunFrame = memo(function RunFrame({
  ...
  const counters = useMemo(() => {
    ...
    const pill = (label: string, className?: string) => (
      <span
        className={cn(
          "flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-muted/40 text-foreground border border-border/50 text-[10px]",
          className
        )}
      >
        {label}
        ...
        <div className="space-y-2 p-3">
          {normalizedInput && (
            <IOFrame
              ...
              defaultOpen={false}
            >
              <pre className="rounded-md bg-muted/50 p-2 text-xs font-mono overflow-x-auto max-h-[200px] overflow-y-auto whitespace-pre-wrap break-words">
                {normalizedInput}
              </pre>
            </IOFrame>
          )}
          ...
          {hasThinking && (
            <IOFrame
              ...
              defaultOpen
            >
              <pre className="rounded-md bg-muted/50 p-2 text-xs font-mono overflow-x-auto max-h-[240px] overflow-y-auto whitespace-pre-wrap break-words">
                {thinkingText}
              </pre>
            </IOFrame>
          )}
