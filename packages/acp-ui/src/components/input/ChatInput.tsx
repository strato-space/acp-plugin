...
const MODE_COLORS: Record<
  string,
  { border: string; button: string; indicator: string; badge: string }
> = {
  code: {
...
export function ChatInput() {
  ...
  const { sendMessage, selectFiles, selectImages, selectMode, cancel } =
    useVsCodeApi();
  ...
  return (
    <div className="sticky bottom-0 w-full overflow-visible rounded-t-3xl bg-background pb-3 sm:pb-4">
      <div className="mx-auto w-full max-w-thread px-1 sm:px-2">
        <div
          ref={dropZoneRef}
        ...
        >
          ...
          >
            ...
            <textarea
              ...
              className="mb-1 max-h-32 min-h-14 sm:min-h-16 w-full resize-none bg-transparent px-3 pt-1.5 pb-3 text-base outline-none placeholder:text-muted-foreground placeholder:opacity-25 focus:outline-none focus:ring-0 focus:border-0"
              rows={1}
              autoFocus
            ...
            <div className="relative mx-1 mt-2 mb-2 flex items-center justify-between">
              ...
              <Button
                ...
                className={cn(
                  ...
                  (!hasContent && !isThinking) || showStop
                    ? ""
                    : modeColors.button
                )}
                aria-label={
                ...
                title={
                  showStop ? "Stop" : isThinking ? "Queue message" : "Send"
                }
              >
                {showStop ? (
