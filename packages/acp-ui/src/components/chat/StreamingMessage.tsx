...
export function StreamingMessage() {
  const { streaming, isThinking, messages, selectedAgentId, agents } = useChatStore();
  ...
  const frameState: ToolState = (() => {
    ...
    if (isThinking || tools.some((t) => t.status === "running")) return "running";
    return "streaming";
  })();
    ...
    >
      <div className="mx-2 leading-normal text-foreground break-words">
        <RunFrame
          ...
          tools={Object.keys(streaming.tools).length > 0 ? streaming.tools : null}
